...
# 1   2   3   4   5   6   7   8
# 0   1   1   1   0   1   0   1
# 이 상황에서 여학생이 3을 받았다면, 3번 스위치를 중심으로 2번, 4번 스위치의 상태가 같고 1번, 5번 스위치의 상태가 같으므로, 1번부터 5번까지 스위치의 상태를 모두 바꾼다.
# 만약 여학생이 4를 받았다면, 3번과 5번의 스위치의 상태가 서로 다르므로 4번 스위치의 상태만 바꾼다.
#
...
# Idea
# 단순 구현 문제네
# 각 학생 액션 함수에서 배수 계산 시, 인덱싱을 정확히 해주는 것에만 유의해서 코드를 짜면 될 것 같다
# 아 옘병 여학생 루프 종료 조건을 잘 설정합쉬다
...
def boys_action(N, idx):
    multiples = idx-1
    # 전체 스위치 배열을 넘지 않도록 세팅
    while multiples < N:
        switch(multiples)
        multiples += idx

def girls_action(N, idx):
    min_idx = idx-1
    max_idx = idx-1
    while True:
        # 여학생 루프의 종료조건 설정
        if min_idx < 0 or max_idx >= N or arr[min_idx] != arr[max_idx]:
            min_idx += 1
            max_idx -= 1
            break
        else:
